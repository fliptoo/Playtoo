...
import models.Playtoo.paginate.strategy.RecordLocatorStrategy;
import play.Logger;
import play.Play;
...
/**
 * Base class to control pagination for a list. Typically, a user would use the
 * {@link JPAPaginator}, {@link ModelPaginator}, or {@link ValuePaginator}
 * implementations.
 * 
 * @param <K>
 * @param <T>
...
public abstract class Paginator<K, T> implements List<T>, Serializable {
	...
	private Paginator() {
		...
		if (params != null) {
			setParameterName(Play.configuration.getProperty(
					"paginator.parameter.name", DEFAULT_PAGE_PARAM));
	...
	public Paginator<K, T> setBoundaryControlsEnabled(
			boolean showBoundaryControls) {
	...
	private static int getLastRowForPage(int startRow, int pageSize,
			int rowCount) {
