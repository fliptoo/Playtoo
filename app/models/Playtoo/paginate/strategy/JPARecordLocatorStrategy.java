...
public class JPARecordLocatorStrategy<K, T> implements RecordLocatorStrategy<T> {
	...
	public JPARecordLocatorStrategy(Class<T> typeToken) {
		...
		String useQueryCacheStr = Play.configuration.getProperty(
				"paginator.jpa.useQueryCache", "true");
	...
	public JPARecordLocatorStrategy(Class<T> typeToken, List<K> keys) {
		this(typeToken);
		String preparedStatementParameters = StringUtils.repeat("?,",
				keys.size());
		preparedStatementParameters = preparedStatementParameters.substring(0,
				preparedStatementParameters.length() - 1);
	 ...
	 * @param params
	 */
	public JPARecordLocatorStrategy(Class<T> typeToken, String filter,
			Object... params) {
		this(typeToken);
		this.filter = filter;
		this.params = params;
	...
	private List<T> findByIndex(int firstRowIdx, int lastRowIdx) {
		...
		List<T> returnMe = query(null, true).setFirstResult(firstRowIdx)
	...
	protected Query query(String select, boolean applyOrderBy) {
		...
		// Play! <= 1.2.3 did not have built-in support for multiple
		// databases...
		// To ensure we are backwards compatible, we use reflection to check
		// that this
		// API is available, so this code still works for people using Play! <=
		// 1.2.3.
		if (typeToken.isAnnotationPresent(PersistenceUnit.class)) {
			String unitName = typeToken.getAnnotation(PersistenceUnit.class)
					.name();
			try {
				Method getJPAConfigMethod = JPA.class.getMethod("getJPAConfig",
						String.class);
				// guard: only call this code if the user is using a version of
				// Play! that
				// has the static getJPAConfig method on the JPA class (Play! <=
				// 1.2.3 does not)
				if (getJPAConfigMethod != null) {
					// use reflection to support Play! <= 1.2.3
					// em = JPA.getJPAConfig(unitName).getJPAContext().em();
					Object config = getJPAConfigMethod.invoke(JPA.class,
							unitName);
					Method getJPAContextMethod = config.getClass().getMethod(
							"getJPAContext");
